...
plant_utilisation_order_count <- data %>%
  distinct(`DOI`, `Plant species`, `Plant utilisation`, `Plant order`) %>%
  group_by(`Plant utilisation`, `Plant order`) %>%
  summarise(Count = n()) %>%
  ungroup()

#Identifying and labelling plant orders that appear only once within each plant utilisation group
plant_utilisation_order_count <- plant_utilisation_order_count %>%
  group_by(`Plant utilisation`) %>%
...
plant_utilisation_order_count_reorder <- plant_utilisation_order_count %>%
  group_by(`Plant utilisation`, Plant_order) %>%
  summarise(Total_Count = sum(Count)) %>%
  ungroup() %>%
  mutate(`Plant utilisation` = factor(`Plant utilisation`, levels = c("Crop", "Model plant", "Other")))
...
plant_utilisation_order_count_reorder <- plant_utilisation_order_count_reorder %>%
  arrange(`Plant utilisation`, -Total_Count) %>%
...
#Creating figure 2
plot_plant_utilisation_order <- ggplot(plant_utilisation_order_count_reorder, aes(x = `Plant utilisation`, y = Total_Count, fill = Plant_order)) +
  geom_bar(stat = "identity", position = "stack") +
  theme_minimal() +
  labs(x = "Plant utilisation",
...
plot_collectiontime_per_approach <- ggplot(collectiontime_per_approach, aes(x = `Sampling duration`, fill = `Sampling approach`)) +
  ...
  theme(plot.title = element_blank(), 
        ...
        axis.ticks = element_line(color = "black")) + 
  scale_fill_manual(values = custom_palette) +  # Apply the custom 10-color palette
  scale_x_discrete(labels = c("<1 min", "2 min", "15 min", "1 h", "2 h", "3 h", "4 h", "6 h", "8 h", "10 h", "12 h", "22 h", "1 d", "1.25 d", "1.71 d", "2 d", "3 d", "6 d", "7 d", "13 d", "14 d", "21 d", "28 d", "32 d", "42 d", "45 d", "56 d", "undefinable", "N.A."), expand = c(0,0)) +
...
```{r Figure 5, Plotting the distribution of annotated metabolites to the total of reported metabolites in percent by analysis method as stacked bar graphs}
#Creating a custom colour-blind friendly colour-palette used in the figures 5+6
npg_colors <- pal_npg("nrc")(4)

#Assigning specific colors to each analysis method 
custom_colors <- c(
  "GC-MS" = npg_colors[1],
  "LC-MS" = npg_colors[2],
  "Mixed" = npg_colors[3],
...
percent_annotated_noNA <- data %>%
  distinct(`DOI`, `Sum of reported compounds`, `Sum of annotated compounds`, `Annotated of reported compounds [%]`, `Analysis method`, `Carbohydrates [n]`, `Fatty acids [n]`, `Amino acids and Peptides [n]`, `Terpenoids [n]`, `Shikimates and Phenylpropanoids [n]`, `Alkaloids [n]`, `Polyketides [n]`, `Other [n]`) %>%
  filter(!is.na(`Annotated of reported compounds [%]`)) 

#Creating figure 5
plot_percent_distribution <- ggplot(percent_annotated_noNA, aes(x = `Annotated of reported compounds [%]`, fill = `Analysis method`)) +
...
```{r Figure 5.1, Plotting the distribution of annotated metabolites to the total of reported metabolites in percent by analysis method as stacked bar graphs (where the number of total annotated compounds is under or equal 100)}
...
#Creating figure 5.1
plot_percent_distribution_u100 <- ggplot(percent_annotated_noNA_u100, aes(x = `Annotated of reported compounds [%]`, fill = `Analysis method`)) +
...
```{r Figure 5.2, Plotting the distribution of annotated metabolites to the total of reported metabolites in percent by analysis method as stacked bar graphs (where the number of total annotated compounds is above 100)}
...
#Creating figure 5.2
plot_percent_distribution_a100 <- ggplot(percent_annotated_noNA_a100, aes(x = `Annotated of reported compounds [%]`, fill = `Analysis method`)) +
  geom_histogram(binwidth = 5, boundary = 0, position = "stack") +  # Stacked histogram
  theme_minimal() +
  labs(title = "Annotated metabolites [%] where -Total reported compounds- >100",
       x = "Annotated metabolites [%]",
       y = "Number of analyses \n(Total reported metabolites >100)",
       fill = "Analysis method") +  # Label for the fill legend
  theme(plot.title = element_blank(), 
        axis.title.x = element_text(size = 17),
        axis.title.y = element_text(size = 17),
        axis.text.x = element_text(size = 12, angle = 45, vjust = 1, hjust = 1),
...
#Filtering out the analysis methods "mixed" and "other", choosing all relevant rows from the data file where the total contribution percentages are under 100 % and calculating the mean, median and standard deviation of all relevant metabolite classes
summary_table_percent_method <- data %>%
  filter(`Analysis method` != "Mixed" & `Analysis method` != "Other") %>%
  distinct(`DOI`, 
...
summary_table_all_methods <- data %>%
  ...
  mutate(`Analysis method` = "All methods combined")


#Ordering the compound classes by the mean from calculations including all analysis methods
ordered_components <- summary_table_all_methods %>%
  arrange(desc(mean)) %>%
  pull(Component)

#Changing the table with excluded analysis methods into long format
...
#Combining both tables with excluded analysis methods and all methods included
...
#Creating figure 6
plot_contr_com_method <- ggplot(combined_summary, aes(x = Component, y = mean, fill = `Analysis method`)) +
...
```{r Figure 7, Plotting the distribution of annotated metabolites to the total of reported metabolites in percent by analysis method as stacked bar graphs}
...
for (mT in 1:nrow(initDB)) {
  ...
  #Data_file_17_09_2024$`New exudation analysis method condensed`
  Data_file_17_09_2024 <- Data_file_17_09_2024 |> 
    filter(`Analysis method` %in% c(methodType)) |> 
